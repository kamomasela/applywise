...
interface StepProgressProps {
  ...
  completedSteps?: number[]; // steps with saved DB data (show green tick regardless of position)
}
    ...
    <div className="mb-8 select-none" aria-label={`Step ${currentStep} of ${STEPS.length}`}>
      ...
      <div className="flex justify-between">
        {STEPS.map((label, i) => {
          ...
          // A step is completed if it's in the DB-derived list, or (fallback) it's before currentStep
          const completed = completedSteps
            ? completedSteps.includes(step) && !active
            : step < currentStep;

          return (
            <Link
              key={label}
              href={`/profile/step-${step}`}
            ...
            >
              ...
              <span
                className={[
                  ...
                  active ? 'text-[#0b4f6c]' : completed ? 'text-[#1ec97e]' : 'text-gray-400',
                ].join(' ')}
              >
                {label}
              </span>
